...
| DB | SQLite at `/config/motif.db`; current schema **v80** (`CURRENT_SCHEMA_VERSION` in `app/core/db.py`), forward-only migrations in `app/core/db.py` |
...
| Bulk PUSH predicates (3 sites) | `app.js:~13909` / `~14066` / `~16236` | pushableCount + pushCount + bulk-PUSH click handler (v1.19.38 fix). Since v0.51.348 all three also exclude the TERMINAL place reasons (`backup_only`, `plex_rejected:over_ceiling`) the row's own `awaitingApproval` excludes — a terminal row cannot be pushed, so counting it promises a failure; `last_place_attempt_reason` therefore rides `_LIB_SELECTION_COLUMNS` (an off-page row without it reads undefined and is offered again) |
...
| library rows (two-phase) | `api.py` `_library_main_query` | since v0.51.345 the query picks the page's identities first and hydrates second; a post-stat filter reads only `_LIB_POST_STAT_COLUMNS` (a new matcher column must join it — `_LibPostStatRow` raises KeyError otherwise), and every predicate/sort keeps its SQL text so the mirror sites here cannot drift. Since v0.51.346 a pill's SQL and post-stat sides read one text: a chip whose predicate is SQL (ATTN update / restore) is projected per request as a 0/1 column from its own constant (`_LIB_ATTN_UPDATE_SQL` / `_LIB_ATTN_RESTORE_SQL`), never re-derived from display columns; each post-stat stage stats only the flag its matcher reads (the page gets the other before hydration) on the `library-stat` pool (16 daemon threads, at most 16 batches of 8 in flight per request, released by `main()` after the drain); `?selection=true` answers SELECT ALL with `_LIB_SELECTION_COLUMNS` only (a node Proxy test fails a bulk read outside them) |
...
- **Every streamed file digest goes through `hash_file`** in `app/core/canonical.py`
  (v0.51.344, reads through `Path.open`); `tests/test_v0_51_344_shared_hash_file.py`
  fails on a second hash loop or a `hashlib.file_digest` in `app/`. A whole-file
  `read_bytes()` digest is outside that ratchet (`api.py`'s `_rehash`, `audio_edit`'s
  candidate shas).
...
- Add a credential-capable `motif.yaml` field without adding it to the ONE
  ...
  v0.51.341) round-trips its mask through PATCH via `unmask_url_credentials` (the userinfo plus the query's and the fragment's secret params, names matched case-blind — v0.51.343). Since v0.51.344 unmask alone decides what a mask is: a masked name takes the stored param of the same spelling first, then case-blind (casefold), and a tab loaded before v0.51.343 that sends back its build's mask of the stored URL (.341/.342's last-"@" shape, .340's first-"@" one) saves it unchanged.
