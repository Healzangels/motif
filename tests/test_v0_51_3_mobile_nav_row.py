...
def test_topbar_is_two_row_areas_grid_on_mobile():
    assert 'grid-template-areas: "brand status" "nav nav";' in MOBILE, \
        "nav must span its own full-width second row (v0.51.161: INBOX back in cluster)"
    # v0.51.161: 2 columns (brand shrink-floor, status 1fr) — INBOX rides the cluster.
...
def test_desktop_topbar_grid_unchanged():
    # v0.51.161: the base rule is a 3-col single row (brand · nav · status; INBOX
    # rides the status cluster) — the 2-row layout is a phone-tier override only
