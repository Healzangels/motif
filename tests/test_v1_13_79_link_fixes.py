...
def test_backfill_banner_link_targets_queue_not_logs():
    ...
    # v0.51.321: to the banner's closing </section>, not a fixed 2000-char window
    # (the .261 ratchet had banked this window's disappearance while the file
    # was empty; restoring it re-anchored rather than re-widened).
    banner = html[block_start:html.index("</section>", block_start)]
...
def test_version_string_matches_current_release():
    ...
    assert '__version__ = "0.51.337"' in init_py, (
