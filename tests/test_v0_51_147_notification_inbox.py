...
def test_fresh_db_has_notifications_table_at_v71(tmp_path):
    ...
    try:
        ...
        # v0.51.168: a fresh DB runs the whole chain → current schema (now v73).
        assert conn.execute("SELECT MAX(version) FROM schema_version").fetchone()[0] == 73
    finally:
        conn.close()
...
def test_migration_v70_to_v71_creates_the_table(tmp_path):
    ...
    try:
        ...
        # current head (v73 as of v0.51.168) — assert full migration, not just v71.
        assert conn.execute("SELECT MAX(version) FROM schema_version").fetchone()[0] == 73
    finally:
        conn.close()
