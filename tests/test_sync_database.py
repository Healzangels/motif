...
def _build_database_tarball(records: dict, *, prefix: str = "ThemerrDB-database") -> bytes:
    ...
    """
    # v1.23.67: production codeload tarballs always ship all three subtrees;
    # backfill any the test omitted with an empty pages.json so the fixture
    # satisfies _extract's all-subtrees completeness gate (the truncation guard).
    records = dict(records)
    for _sub in ("movies", "tv_shows", "movie_collections"):
        records.setdefault(_sub, {})
    buf = io.BytesIO()
...
def test_snapshot_acquire_and_read(db_path: Path):
    ...
    try:
        ...
        # v1.21.38: index() now returns (items, failed_pages).
        movies_idx, movies_failed = snap.index("movies")
        tv_idx, tv_failed = snap.index("tv_shows")
        assert len(movies_idx) == 1 and movies_idx[0]["id"] == 100
        assert len(tv_idx) == 1 and tv_idx[0]["id"] == 200
        assert movies_failed == 0 and tv_failed == 0
...
# ---------- truncation (v1.23.67, audit #14) ---------------------------------

def test_snapshot_rejects_truncated_tarball(db_path: Path):
    """A codeload tarball truncated to ONLY a complete movies/ subtree
    (tv_shows/ + movie_collections/ absent — an interrupted/edge-corrupt
    download that still extracts cleanly) must raise _SnapshotError so run_sync
    falls through to the git/remote tier, NOT fake-succeed with a zero-theme
    tv/collection sync. Built inline because _build_database_tarball now
    backfills all three subtrees."""
    from app.core import progress as op_progress
    from app.core.sync import _DatabaseSnapshot, _SnapshotError
    ...
    with tarfile.open(fileobj=buf, mode="w:gz") as tf:
        ...
        di.mode = 0o755
        tf.addfile(di)
        _add_file(tf, f"{prefix}/movies/pages.json",
                  json.dumps({"count": 1, "pages": 1, "imdb_count": 1}).encode())
        _add_file(tf, f"{prefix}/movies/all_page_1.json",
                  json.dumps([{"id": 100, "imdb_id": "tt0000001",
                               "title": "T"}]).encode())
    client = _make_client(_StaticHandler(body=buf.getvalue()))
    snap = _DatabaseSnapshot(
        db_path, "tdb_sync",
        "https://codeload.invalid/tar.gz/database",
        cancel_check=lambda: False,
