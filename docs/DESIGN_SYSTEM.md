...
Type scale:
  --t-micro  9px   carets, SRC/LINK badges, TDB + loudness pills, micro captions, op-card labels
...
No font-size in app.css or ops.css retypes a step's px value; it reads the token (v0.51.343, `tests/test_v0_51_343_type_scale_micro.py`; the one pinned exception is the `.dlg-close` 22px glyph).
...
`// SYNC THEMERRDB`, `// EXPORT CSV`). Tracking `--track: 0.15em` (the v0.51.332 convention),
uppercase, mono font.
...
* `.link-badge` + `.link-badge-X` — source-attribution pill (T/A/U/M/P palette). One canonical family across library, info card, and dashboard donut. Its `letter-spacing` trails the last glyph, so the rule declares `--track: 0.15em` and both `letter-spacing: var(--track)` and the right padding `calc(6px - var(--track))` read it (v0.51.331, the v1.12.30 `.link-glyph` idiom; one variable since v0.51.343). The SRC key buttons (`.link-badge.src-key-btn`) take the same padding, since `.src-key-btn` declares none.
* `.state-pill` + `.state-pill-pending` — row state dots (DL/PL). Pulse-amber for in-flight.
* `.title-glyph` + `.row-play` (v0.51.333) — the library row's leading play slot: a title glyph reserved on every row (18px flex item, no tracking, empty `.row-play-none` span when nothing plays), `■` in green with the family's glow while playing (`.row-play-on`). What it plays is `lib/quick-play.js` (mirrors the INFO card headline); one shared `#row-quick-play` element, one sound at a time both ways. Tooltip voice: `Play — motif's file (placed)` / `(not placed yet)` / `Play — what Plex serves` / `(motif's copy stands by)`; `Stop` while playing. `.now-playing` (v0.51.334) is the header strip — label · title button · tabular clock · the same ■ — a composite pill (dashed `--line-bright`, no tracking of its own to compensate), hidden until a row plays. It needs its `.now-playing[hidden] { display: none; }` companion (v0.51.338): the strip's `inline-flex` outranks the UA rule, and .337 shipped without it.

**Convention (v0.51.332; one variable since v0.51.343): tracked labels give the trailing letter-spacing back.** `letter-spacing` is laid after the last glyph too, so a centred text box carries one tracking unit of dead space on its right and the ink sits half of it left of centre. Each family below therefore declares its tracking once, as `--track`, and both its `letter-spacing` and its right padding read it: `--track: 0.1em; letter-spacing: var(--track); padding: 2px calc(8px - var(--track)) 2px 8px;` (right = left − the track). The families are `.btn` / `.btn-tiny` / `.row-info-btn`, `.chip` (+ the section chips), `.tab`, `.tdb-pill`, `.loudness-pill`, `.attn-pill`, `.pill-filter-clear`, the filter row's `.link-glyph` / `.ed-pill-btn`, `.info-scope-chip`, `.tier-badge`, `.pill`, `.lib-flag-pill`, `.edition-pill`, `.form-env-badge`, `.sync-hist-status`, `.dash-section-toggle` / `.dash-card-toggle`, `.library-filter-toggle`, `.library-clear-all-btn`, `.link-badge` and the per-kind LINK glyphs (`.link-glyph-hardlink` … `.link-glyph-ab`). `.brand-update`, `.history-level`, `.sync-hist-transport` and `.input::file-selector-button` still carry a literal `letter-spacing` with symmetric padding: moving them onto `--track` and the calc (and into `TRACKED`) shifts their ink by half a track, a visible change not made (v0.51.344). `.btn-tiny`, `.row-info-btn` and the section chips (`.chips[aria-label="section"] .chip`) declare no tracking of their own and read their owner's `--track`, so the element must carry the owner class: `btn btn-tiny`, never a bare `btn-tiny`. A composite pill gives the tracking back on its last run: `.op-pill-label { --track: 0.16em; letter-spacing: var(--track); margin-right: calc(-1 * var(--track)); }`. To retune a family, change its `--track`, never its `letter-spacing` or the calc. A new tracked pill takes the same shape and joins `TRACKED` in `tests/test_v0_51_332_pill_ink_centred_sweep.py`. The sweep asserts the mechanism for each family (one `letter-spacing`, reading `--track`), and its discovery test fails on any give-back padding that is not listed in `TRACKED` / `INHERITED`. Since v0.51.344 discovery reads app.css and ops.css, `@media` rules included, every selector of a shared list and every declaration (`!important` dropped), and counts as a give-back a right padding that subtracts `var(--track)` or an em, any right side that is `calc(<the rule's own left> − anything)` — a px literal or another custom property too — and a lone `padding-right` calc that subtracts in a rule with no left padding of its own; such a pill is listed and moved onto `--track`. Glyph-only pills whose glyph sits off the box centre (the DL/PL `●` / `—` buttons, the `↺ ⟳ ⚠` attention symbols) are lifted by moving vertical padding, never by changing height. The `▂▄▆` loudness bars are bottom-anchored by design. A lone-glyph label with symmetric padding drops the tracking instead (`letter-spacing: 0`): the `⏻` `.topbar-logout` and the loudness stepper's `–` / `+` (`.loud-stepper .btn-tiny`, v0.51.339).
* `.tdb-pill`, `.attn-pill`, `.op-pill` — semantic pills carrying status counts in the topbar.
* `.tier-badge` + `.tier-badge-X` — small mono chips (v0.51.324 doc). In the INFO card title: `-4k` (library version), `-lvl` / `-loud` / `-raw` (loudness state). In the AUDIO group's play rows (v0.51.323; since v0.51.340 under the row's label in the `<dt>`, so every player starts at the value edge): `-serving` (amber — what Plex plays), `-standing` (cyan — a backup-intent file waiting), `-placed` (green — motif's file deployed), `-unplaced` (dim outline). In the backup list (v0.51.335): `-bundle` (green), `-snapshot` (dim), `-prerestore` (amber) — the kind chip that leads each row; an uploaded or a partial bundle (`motif-bundle-partial-…`, a member left out over its cap, v0.51.344) keeps `-bundle` and says so in the chip's tooltip. Colour ENCODES state — theme SPLIT, not chrome.
* `.info-scope-chip` + `-section` / `-edition` — the INFO card hero's scope chips under the title (which library section, which edition).
* `.info-ctl-label` / `.info-ctl-label-play` (v0.51.340) — on a `.dlg-grid` `<dt>` whose value starts with a control: the label centres on the value's FIRST line (`--btn-tiny-h` for a button line, `--info-player-h` for a player, which `.info-audio` also reads), so a wrapped value never drags it down. The play variant stacks its state `.tier-badge` under the label text (keep a space between them). The INFO card's fold caret hangs off `.history-section-title::before` into the gutter, so fold titles start at the group titles' x; a `.history-section-title` must open its `<summary>`.
...
* **`<p class="form-subhead">`** for inline sub-section headers within a single block (16 sites at v0.51.263 — originally INTEGRATION layouts; the NOTIFICATIONS relayout added five family headers).
...
* **The phantom-whitespace void.** `.block-body` carries `white-space: pre-wrap` (other consumers depend on it), so the SOURCE indentation between settings controls rendered as real vertical gaps that varied with how each block's Jinja happened to be indented. Fix: `.tab-panel .block-body` switches to `white-space: normal` + becomes a `display: flex; flex-direction: column; gap: var(--gap-5)` — the GAP owns ALL inter-child spacing, so every section breathes identically regardless of source indentation. Per-child `margin-top/bottom` are zeroed (`.tab-panel .block-body > *`) so nothing stacks on top of the gap. `.tab-panel .form-grid` is pinned to one density (`gap-5`) — the old tab-by-tab `form-grid` vs `form-grid-tight` split is gone for tab-panels — `.tab-panel .form-grid` outranks `-tight`'s gap, so the class is INERT for spacing where it still appears on tab-panel markup (four grids carry it, incl. the v0.51.263 inbox grid; only its `.form-hint` spacing rule has any effect there). Live use: the new-token DIALOG, a different surface.
...
* **Checkbox stacks + prose stay full-width.** `.form-checkbox` toggles and bare narrative `.form-hint` keep `form-label` / full width — the hybrid rule. Don't force a checkbox into a `.field-row`. (Hint LENGTH is governed separately: since v0.51.263 the NOTIFICATIONS hints are one line each — see the grouped-stacks section below.)
...
* **`[hidden]` + a `display` class don't mix.** A view-scoped element toggled via the `hidden` attribute (the LOGS LIVE indicator) needs an explicit `.X[hidden] { display: none }` rule, because a class's `display: flex` outranks the UA `[hidden]{display:none}` and the element leaks onto the other view. `tests/test_v0_51_338_library_ui.py` enforces this for every hidden element in every template (v0.51.338).
...
The snippets above are the v1.17.6 shape. Since then the base rule is `overflow: visible` (v1.17.21, so the panel can drop out of the bar) with `max-width: 100%; min-width: 0` (v1.18.44), and the overflow test is `_barHasOverflow` (v1.20.31): the rightmost visible child's painted edge against the bar's inner edge, because `scrollWidth` does not grow reliably on an `overflow: visible` flex row.

**v0.51.344 follow-up — wrap on a narrow screen, lay out again when a label changes.** At phone widths the primaries plus `// MORE ▾` cannot share one row (at 375px they painted out to about 660px; two probes read 660 and 661). Each `_layoutBulkBar()` pass starts on one row (removes `.is-wrapped`), parks overflow buttons in the panel as before, and adds `.is-wrapped` when the bar still overflows — the layout measures, so the state is styled at TOP LEVEL, never inside a breakpoint:

```css
#library-bulk-bar.is-wrapped { flex-wrap: wrap; position: relative; }
#library-bulk-bar.is-wrapped > .btn { max-width: 100%; white-space: normal; }
#library-bulk-bar.is-wrapped > .row-menu { position: static; }
#library-bulk-bar.is-wrapped .row-menu-panel { left: 0; right: auto; max-width: 100%; }
```

An in-bar label wraps inside its own button (panel buttons keep one line), and the panel opens left-aligned below the bar. A `MutationObserver` on the bar (`childList`, `subtree`) re-runs the layout when any button's text changes: a bulk result label written after layout grew its button past the bar, which the `ResizeObserver` on the bar's box never saw. Layout's own moves target the bar or the panel, never a button, so they cannot re-trigger it. A running button (disabled, handler-owned) is never a `// MORE` candidate, so its progress and result stay in sight, and a panel button goes back before its template successor (`_bulkBarOrder`), so the bar settles in template order after a run. While `// MORE` is open, `_layoutBulkBar()` defers instead of running (the reset would empty the open panel mid-press — a lost click, focus to BODY, scrollTop to 0): it sets `_bulkBarLayoutDeferred` and returns, so a running action's progress writes never rebuild the menu, and the deferred pass runs once on the `<details>` `toggle` to closed (the close-on-action hook fires that toggle). Trade-off: at rest the one-row contract holds from 768px up, but the wrap is measured, not width-gated, so a long enough running or result label can wrap the bar at any width — 56 → 90px at 768px for `// RESTORING i/30`, and 141px at 768px / 90px at 1280px for a long failure label — and the table below shifts for as long as the action runs. Panel labels stay on one line on purpose, and the wrapped panel is capped at the bar's width (`max-width: 100%` on `#library-bulk-bar.is-wrapped .row-menu-panel`; `#library-bulk-overflow-menu .row-menu-panel` already has `overflow-y: auto`), so a label wider than a 360–375px bar scrolls inside the panel instead of painting past the viewport — before the cap an open `// MORE` painted 364px at a 360px viewport with three-digit counts and 381px at 375px with five-digit ones. Other components may reuse the `is-wrapped` state-class name; `tests/test_v0_51_344_bulk_bar_wrap.py` scans only the bar's own `#library-bulk-bar.is-wrapped` rules for breakpoints, and `tests/test_v0_51_344_bulk_bar_open_panel.py` pins the open-panel deferral.
...
* Failure-related copy: `// TRY THIS NEXT` (live) / `✓ RESOLVED` / `✓ ACKED` / `✓ PLEX SERVES — OPTIONAL UPGRADES` (resolved-via-plex) / `✓ BACKUP READY — DEFERRING TO PLEX` while Plex serves the item, `✓ BACKUP READY — PLEX SERVES NO THEME` when Plex serves no theme (has_theme 0 or a verify 404; nothing records that it once served, so never "no longer" — v0.51.344; the headline, the STANDING BY tooltip and the PROMOTE caption use the same phrase; with nothing on disk the headline reads `Plex serves no theme (its last verify found none)` for a verify 404 under has_theme 1, and says "Plex serves" only while the card's Plex player shows — `_derivePlaybackSourceLabel` keys on `_plexBackupState(data) === 'serves'`, the player's own reading, never on has_theme alone), `✓ BACKUP READY — NOT IN PLEX` when the item left (user explicitly chose backup intent via KEEP AS BACKUP, v1.18.77+; the three titles, the PROMOTE caption and the STANDING BY tooltip key on `_plexBackupState`, v0.51.343).
* Intent-flip button pair (v1.18.77+; since v0.51.323 it lives in the state strip's HEADER, right of the title, and its caption is the strip's one note line): `// PROMOTE TO ACTIVE` (flip backup → replace, force-place motif's URL) + `// MARK AS BACKUP` (flip replace → backup, stop trying to push past plex_has_theme). MARK AS BACKUP is `.btn-warn` (amber, mutating); PROMOTE TO ACTIVE carries a source-toned `.btn-promote-pb` / `.btn-promote-tb` / `.btn-promote-ab` / `.btn-promote-ub` (v1.19.86 — the tone names what gets deployed: Plex-cloud, ThemerrDB, adopted, user). Both change `user_overrides.intent` and trigger worker-side state changes. PROMOTE shows whenever intent='backup' and the item is in Plex (`_plexBackupState` ≠ 'absent', v0.51.344); its tooltip says "over Plex's theme" only while Plex serves one; MARK AS BACKUP only when intent='replace' AND `data.plex_resolved` (Plex has a fallback theme to serve).
