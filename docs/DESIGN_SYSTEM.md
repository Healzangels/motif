...
* Failure-related copy: `// TRY THIS NEXT` (live) / `✓ RESOLVED` / `✓ ACKED` / `✓ PLEX SERVES — OPTIONAL UPGRADES` (resolved-via-plex) / `✓ BACKUP READY — DEFERRING TO PLEX` while Plex serves the item, `✓ BACKUP READY — PLEX NO LONGER SERVES` once it stops (has_theme 0 or a verify 404), `✓ BACKUP READY — NOT IN PLEX` when the item left (user explicitly chose backup intent via KEEP AS BACKUP, v1.18.77+; the three titles, the PROMOTE caption and the STANDING BY tooltip key on `_plexBackupState`, v0.51.343).
