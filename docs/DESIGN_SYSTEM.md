...
**Convention (v0.51.332): tracked labels give the trailing letter-spacing back.** `letter-spacing` is laid after the last glyph too, so a centred text box carries one tracking unit of dead space on its right and the ink sits half of it left of centre. Every boxed, centred, tracked label therefore declares `padding-right = padding-left − letter-spacing` (`calc(8px - 0.1em)` etc.) — `.btn` / `.btn-tiny` / `.row-info-btn`, `.chip` (+ the section chips), `.tab`, `.tdb-pill`, `.loudness-pill`, `.attn-pill`, `.pill-filter-clear`, the filter row's `.link-glyph` / `.ed-pill-btn`, `.info-scope-chip`, `.tier-badge`, `.pill`, `.lib-flag-pill`, `.edition-pill`, `.form-env-badge`, `.sync-hist-status`, `.dash-section-toggle` / `.dash-card-toggle`, `.library-filter-toggle`, `.library-clear-all-btn`; a composite pill compensates on its last run (`.op-pill-label { margin-right: -0.16em }`). A new tracked pill must do the same; `tests/test_v0_51_332_pill_ink_centred_sweep.py` parses each rule. Glyph-only pills whose glyph sits off the box centre (the DL/PL `●` / `—` buttons, the `↺ ⟳ ⚠` attention symbols) are lifted by moving vertical padding, never by changing height. The `▂▄▆` loudness bars are bottom-anchored by design. A lone-glyph label with symmetric padding drops the tracking instead (`letter-spacing: 0`): the `⏻` `.topbar-logout` and the loudness stepper's `–` / `+` (`.loud-stepper .btn-tiny`, v0.51.339).
