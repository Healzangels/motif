...
#library-bulk-bar.is-wrapped .row-menu-panel { left: 0; right: auto; }
```

An in-bar label wraps inside its own button (panel buttons keep one line), and the panel opens left-aligned below the bar. A `MutationObserver` on the bar (`childList`, `subtree`) re-runs the layout when any button's text changes: a bulk result label written after layout grew its button past the bar, which the `ResizeObserver` on the bar's box never saw. Layout's own moves target the bar or the panel, never a button, so they cannot re-trigger it. A running button (disabled, handler-owned) is never a `// MORE` candidate, so its progress and result stay in sight, and a panel button goes back before its template successor (`_bulkBarOrder`), so the bar settles in template order after a run. Trade-off: at rest the one-row contract holds from 768px up, but the wrap is measured, not width-gated, so a long enough running or result label can wrap the bar at any width — 56 → 90px at 768px for `// RESTORING i/30`, and 141px at 768px / 90px at 1280px for a long failure label — and the table below shifts for as long as the action runs. Gap: the bar and its labels stay inside the page at 375px and up, but because panel labels stay on one line and the wrapped panel is left-anchored with no `max-width`, an open `// MORE` panel still paints past the edge once its counts are long (measured: 364px at a 360px viewport with three-digit counts, 381px at 375px with five-digit ones — the parent painted past the edge in every one of those cases, so this is strictly better, but the operator's own overflow reports came from about 361px). Closing it means `max-width: 100%` on `#library-bulk-bar.is-wrapped .row-menu-panel`, which then scrolls inside the panel. Other components may reuse the `is-wrapped` state-class name; `tests/test_v0_51_344_bulk_bar_wrap.py` scans only the bar's own `#library-bulk-bar.is-wrapped` rules for breakpoints.
